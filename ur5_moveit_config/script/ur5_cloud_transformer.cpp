...
/*
* Brief:
* This node transforms point cloud from /kinect_link frame to /world frame with noise
*/
...
public:
      ...
      demo_(0)
  {
    ...
    pcl_sub_ = nh_.subscribe("pcl_publish", 1, &CloudTransformer::pclCallback, this);
    pcl_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/ur5/world/points", 1);

    buffer_.reset(new sensor_msgs::PointCloud2);
    buffer_->header.frame_id = "world";

    if(ros::param::get("/ur5_cloud_transformer/demo", demo_))
    {
...
private:
  ...
  void pclCallback(const sensor_msgs::PointCloud2ConstPtr& pcl_msg)
  {
    ...
    listener_.waitForTransform("world", "world", ros::Time::now(), ros::Duration(1.0));
...
{
  ros::init(argc, argv, "ur5_point_cloud_tf");
  ros::NodeHandle nh;
